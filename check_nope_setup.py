...
# נשתמש בפונקציית האימות שלכם שכבר קיימת ב-server/nope_utils.py
try:
    from server.nope_utils import verify_nope_token_file
except Exception as e:
    print("❌ לא הצלחתי לייבא verify_nope_token_file מ-server/nope_utils.py:", e)
    sys.exit(2)
...
def find_token_for_server(server_id: str) -> Path | None:
    # מעדיפים JSON "אמיתי", ואם אין - מחפשים legacy .tok
...
def load_server_public_key(server_id: str):
    ...
    raise FileNotFoundError(f"לא מצאתי cert.pem עבור {server_id} ב: {TLS_CANDIDATES}")
...
def check_tokens(server_ids=("S1","S2","S3")) -> bool:
    ...
    for sid in server_ids:
        ...
        if not token_path:
            print(f"❌ {sid}: לא נמצא טוקן ב-nope/tokens ({sid}.nope.json או {sid}.tok)")
            all_ok = False
            continue
        try:
            pubkey, cert_path = load_server_public_key(sid)
        except Exception as e:
            print(f"❌ {sid}: בעיית TLS - {e}")
            all_ok = False
            continue
    ...
    return all_ok
def main():
    # ZK נבדוק עם snarkjs ב-PowerShell; כאן נטפל בטוקנים בלבד
    print("=== בדיקת טוקני NOPE מול TLS ===")
    ok = check_tokens(("S1","S2","S3"))
    print("סיכום:", "✅ OK" if ok else "❌ FAILED")
